/**
 * @name markerX
 * @class This class includes has functions to add markers to the map.
 *
 * @param {Google map object} map
 * @param {json} options includes required parameters for creating markers (Thumbnail or normal marker).
 * Currently, in order to add custom markers, the path should be directly used here (Needs changing)
 */
...
*/
    ...
    if (this.clusterSize > 1)
        this.addLabel(jsonObject.style, jsonObject.type, jsonObject.color, jsonObject.thumb, this.thumbPosition);
}
...
function getIconForMarkerParamType(jsonObject) {
    ...
    if (jsonObject.style == "thumbnail") {
        ...
        size = new google.maps.Size(jsonObject.width *2, jsonObject.height*2);
        origin = new google.maps.Point(0, 0);
        anchor = new google.maps.Point(jsonObject.width / 2, jsonObject.height);
        scaledSize = new google.maps.Size(jsonObject.width * 2, jsonObject.height * 2);
    }
...
markerX.prototype.zoomIntoCluster = function () {
...
markerX.prototype.addLabel = function (markerStyle, type, color, thumb, thumbPosition) {
    var map = this.map;
    this.thumb = thumb;
